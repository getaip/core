...
fn check_private_sdk_boundary() -> Result<(), String> {
    ...
    for path in &required_sdk_paths {
    ...
    eprintln!(
        "private SDK boundary: PASS ({} SDK packages, {} SDK root files, {} connector-repository packages, {} connector root files)",
        sdk_order.len(),
        required_sdk_paths.len(),
...
fn private_sdk_inventory(output: &Path) -> Result<(), String> {
    ...
    let required_paths = required_unique_string_array(&config, "private_sdk_required_paths")?;
    let metadata = cargo_metadata(false)?;
    let workspace_root = metadata_workspace_root(&metadata)?;
